...
module.exports = class AuthService{
    ...
    static authenticateUser(data){
        ...
        .then(user=>{
            if(!user){
                throw new AuthenticationError("Incorrect email address",{path:'email',value:email,field:'body'})
            }
    ...
    static forgotPassword(data){
     ...
     .then(user=>{
        if(!user){
            throw new AuthenticationError("No user found",{path:'email',value:email,field:'body'})
        }
    ...
    static resetPassword(data){
        ...
        .then(user=>{
            if(!user || new Date(Date.now())>user.resetTokenExpires){
                throw new AuthenticationError('Invalid reset token',{path:'token',field:'params',value:resetToken})
            }
            return user['dataValues']
        })
    }
    static changePassword(data){
        ...
        return  User.findOne({where:{resetToken,resetTokenExpires:{
            [Op.gt]:new Date(Date.now())
        }}})
        .then(user=>{
         if(!user){
            throw new AuthenticationError("Invalid reset token",{path:'token',value:resetToken,field:'params'})
         }
